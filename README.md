...
M3 默认使用内存 Fake Executor，不会在开发机上真正启动训练。它会模拟四张目标服务器 GPU（2 张 RTX 3090、2 张 RTX 4090）和训练进度，让控制面可先在 Windows 上完整联调。Linux 部署时将 `EXECUTOR_BACKEND` 切换为 `docker`，即可使用 Docker Engine 和 NVIDIA Container Toolkit 执行真实任务。
...
- 按型号申请 1-2 张 GPU，同一任务不混用 3090/4090。
